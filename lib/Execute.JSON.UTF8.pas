...
(*
  TypInfo based JSON parser/builder for Delphi (c)2015-2024 by Execute SARL
...
type
  ...
  private
    ...
    procedure ParseJSONGUID(var GUID: TGUID);
...
begin
  ...
  case JSONInstanceType(TypeInfo, Instance) of
    ...
    jsGUID     : AppendString(Copy(GUIDToString(TGUID(Instance^)), 2, 36));
  else
...
begin
  case JSONType(TypeInfo) of
    ...
    jsGUID     : ParseJSONGUID(PGUID(Instance)^);
  else
...
procedure TJSONParser.ParseJSONGUID(var GUID: TGUID);
begin
  var str := GetString;
  if str = '' then
...
{$ENDIF}
  case JSONInstanceType(TypeInfo, Instance) of
    ...
    jsSet :
    begin
      Result := JSON.DefaultSet or (TypeInfo.GetOrd(Instance) <> 0);
    end;
  end;
end;
